...
exports.uploadBatchExcel = async (req, res) => {
  try {
    ...
    if (existingBatchFile) {
      return res.status(400).json({ message: 'A batch file has already been uploaded. Please delete the existing file before uploading a new one.' });
    }
    ...
    const batchDataWithMKSU = worksheet.map(batchRow => {
      ...
      const masterRow = masterData.find(masterRow => String(masterRow.ItemCode).trim() === batchItemCode);
